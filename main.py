...
def check_haarcascadefile():
    exists = os.path.isfile(os.path.join(BASE_DIR, "haarcascade_frontalface_default.xml"))
    if exists:
        pass
    else:
...
def save_pass():
    assure_path_exists(os.path.join(BASE_DIR, "TrainingImageLabel"))
    exists1 = os.path.isfile(os.path.join(BASE_DIR, "TrainingImageLabel", "psd.txt"))
    if exists1:
        tf = open(os.path.join(BASE_DIR, "TrainingImageLabel", "psd.txt"), "r")
        key = tf.read()
    else:
        master.destroy()
        ...
        if new_pas == None:
            ...
            tf = open(os.path.join(BASE_DIR, "TrainingImageLabel", "psd.txt"), "w")
            tf.write(new_pas)
    ...
    if (op == key):
        if(newp == nnewp):
            txf = open(os.path.join(BASE_DIR, "TrainingImageLabel", "psd.txt"), "w")
            txf.write(newp)
        else:
...
def TakeImages():
    ...
    assure_path_exists(os.path.join(BASE_DIR, "StudentDetails"))
    assure_path_exists(os.path.join(BASE_DIR, "TrainingImage"))
    serial = 1
    exists = os.path.isfile(os.path.join(BASE_DIR, "StudentDetails", "StudentDetails.csv"))
    if exists:
        df = pd.read_csv(os.path.join(BASE_DIR, "StudentDetails", "StudentDetails.csv"))
        if not df.empty:
            serial = int(df['SERIAL NO.'].max()) + 1
        else:
            serial = 1
    else:
        with open(os.path.join(BASE_DIR, "StudentDetails", "StudentDetails.csv"), 'w', newline='') as csvFile1:
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        harcascadePath = os.path.join(BASE_DIR, "haarcascade_frontalface_default.xml")
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                filename = name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg"
                cv2.imwrite(os.path.join(BASE_DIR, "TrainingImage", filename), gray[y:y + h, x:x + w])
                # display the frame
...
def TrainImages():
    ...
    harcascadePath = os.path.join(BASE_DIR, "haarcascade_frontalface_default.xml")
    detector = cv2.CascadeClassifier(harcascadePath)
    faces, ID = getImagesAndLabels(os.path.join(BASE_DIR, "TrainingImage"))
    try:
        recognizer.train(faces, np.array(ID))
    except:
        mess._show(title='No Registrations', message='Please Register someone first!!!')
        return
    recognizer.save(os.path.join(BASE_DIR, "TrainingImageLabel", "Trainner.yml"))
...
def TrackImages():
    check_haarcascadefile()
    assure_path_exists(os.path.join(BASE_DIR, "Attendance"))
    assure_path_exists(os.path.join(BASE_DIR, "StudentDetails"))
    ...
    exists3 = os.path.isfile(os.path.join(BASE_DIR, "TrainingImageLabel", "Trainner.yml"))
    if exists3:
    ...
    exists1 = os.path.isfile(os.path.join(BASE_DIR, "StudentDetails", "StudentDetails.csv"))
    if exists1:
        df = pd.read_csv(os.path.join(BASE_DIR, "StudentDetails", "StudentDetails.csv"))
    else:
    ...
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile(os.path.join(BASE_DIR, "Attendance", "Attendance_" + date + ".csv"))
    if exists:
        with open(os.path.join(BASE_DIR, "Attendance", "Attendance_" + date + ".csv"), 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
        ...
        with open(os.path.join(BASE_DIR, "Attendance", "Attendance_" + date + ".csv"), 'a+') as csvFile1:
            ...
            writer.writerow(attendance)
        csvFile1.close()
    with open(os.path.join(BASE_DIR, "Attendance", "Attendance_" + date + ".csv"), 'r') as csvFile1:
